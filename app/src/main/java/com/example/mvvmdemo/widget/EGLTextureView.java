 ...
 * A EGLSurfaceView must be notified when to pause and resume rendering. EGLSurfaceView clients
 * are required to call {@link #onPause()} when the activity stops and
 * {@link #onResume()} when the activity starts. These calls allow EGLSurfaceView to
...
public class EGLTextureView extends TextureView implements TextureView.SurfaceTextureListener {
     ...
     * EGLSurfaceView to continue rendering, such as in response to
    ...
    private static class EglHelper {
        public EglHelper(WeakReference<EGLTextureView> EGLSurfaceViewWeakRef) {
        ...
        public void start() {
            ...
            EGLTextureView view = mEGLSurfaceViewWeakRef.get();
            if (view == null) {
                mEglConfig = null;
                mEglContext = null;
            } else {
        ...
        private WeakReference<EGLTextureView> mEGLSurfaceViewWeakRef;
        EGL10 mEgl;
    ...
    static class GLThread extends Thread {
        GLThread(WeakReference<EGLTextureView> EGLSurfaceViewWeakRef) {
            super();
            mWidth = 0;
            mHeight = 0;
            mRequestRender = true;
